...
internal class SensorsReader: Reader<Sensors_List> {
    ...
    public override func read() {
        for i in self.list.sensors.indices {
            ...
            if self.list.sensors[i].group == .unknown { continue }
            autoreleasepool {
                var tmpval: Double? = SMC.shared.getValue(self.list.sensors[i].key) ?? 0
                self.list.sensors[i].value = tmpval!
                tmpval = nil
            }
        }
        ...
        let fanSensors = self.list.sensors.filter({ $0.type == .fan && !$0.isComputed }).map{ $0.value }
        ...
        if self.HIDState {
            ...
            let socSensors = list.sensors.filter({ $0.key.hasPrefix("SOC MTR Temp") }).map{ $0.value }
